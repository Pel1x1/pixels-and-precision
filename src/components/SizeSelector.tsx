  ...
  return (
    <div className={`grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 text-black font-normal text-center ${className}`}>
      {sizes.map((size, index) => (
        <button
          key={index}
          className={`text-base sm:text-lg lg:text-xl xl:text-2xl px-4 sm:px-6 lg:px-8 xl:px-12 py-1 lg:py-2 border-2 transition-all duration-300  whitespace-nowrap ${
            selectedSize === size
